...
sigma_AOA_theta_fixed = 0.2;       % 固定角度噪声 (度)
sigma_AOA_phi_fixed = 0.5;       % 固定角度噪声 (度)
...
for idx = 1:length(sigma_t_values)
    ...
    sigma_AOA_theta = deg2rad(sigma_AOA_theta_fixed); % 转换为弧度
    sigma_AOA_phi = deg2rad(sigma_AOA_phi_fixed); % 转换为弧度
    % 预存储估计结果
    ...
    for mc = 1:numMC
        ...
        % 两阶段定位算法
        u_est = two_stage_estimator(BR_meas, AOA_theta_meas, AOA_phi_meas, Tx_pos, Rx_pos, sigma_r, sigma_AOA_theta, sigma_AOA_phi);
...
function CRLB = compute_CRLB(u, Tx, Rx, sigma_r, sigma_AOA_theta, sigma_AOA_phi)
    ...
    for j = 1:N
        ...
        FIM = FIM + (grad_phi*grad_phi')/(sigma_AOA_theta^2)+...
        (grad_theta*grad_theta')/(sigma_AOA_phi^2) ;
...
function u_est = two_stage_estimator(BR_meas, AOA_theta, AOA_phi, Tx, Rx, sigma_r, sigma_AOA_theta, sigma_AOA_phi)
    M = size(Tx,1);
    N = size(Rx,1);
...
for k = 1:3  % 重复以提高加权矩阵W1  
    ...
    Omega_1 =zeros(3,3);
    Omega_1(1,1) = sum(cos(AOA_theta).^2)*Q(M*N+1,M*N+1);
    Omega_1(1,2) = sum(cos(AOA_theta).*sin(AOA_theta))*Q(M*N+1,M*N+1);
    Omega_1(2,1) = sum(cos(AOA_theta).*sin(AOA_theta))*Q(M*N+1,M*N+1);
    Omega_1(2,2) = sum(sin(AOA_theta).*sin(AOA_theta))*Q(M*N+1,M*N+1);


    Omega_2 = zeros(3,M*N+1);
    for i = 1:N
        Omega_2(1,M*N+1) = Omega_2(1,M*N+1)+[cos(AOA_theta(i)),sin(AOA_theta(i)),0]*Rx(i,:)'*cos(AOA_theta(i))*Q(M*N+1,M*N+1);
    end
    for i = 1:N
        Omega_2(2,M*N+1) = Omega_2(2,M*N+1)+[cos(AOA_theta(i)),sin(AOA_theta(i)),0]*Rx(i,:)'*sin(AOA_theta(i))*Q(M*N+1,M*N+1);
    end
    Omega_3 = Omega_2';
...
function W1 = compute_W1(u, Tx, Rx, BR_meas, AOA_theta, AOA_phi, sigma_r, sigma_AOA_theta, sigma_AOA_phi)
    M = size(Tx,1);
    N = size(Rx,1);
