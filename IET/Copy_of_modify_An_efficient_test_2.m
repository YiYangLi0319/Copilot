...
function CRLB = compute_CRLB(u, Tx, Rx, sigma_r, sigma_AOA)
    ...
    for j = 1:N
        ...
        FIM = FIM + (grad_phi*grad_phi')/(sigma_AOA^2)+...
...
function u_est = two_stage_estimator(BR_meas, AOA_theta, AOA_phi, Tx, Rx, sigma_r, sigma_AOA)
    ...
    rd_hat = [reshape(BR_meas',[N,1]); AOA_theta;AOA_phi]';
%     W1 = inv(B * Q * B');
    W1 = inv(Q);
...
for k = 1:3  % 重复以提高加权矩阵W1  
    ...
    Omega_12 =[];
    for i = 1:N
        Omega_12 = [Omega_12;2*Q(i,i)*W1(i,i)*(rd_hat(i)+ d_ij(1,i))];
    end
    Omega_21 = Omega_12';
    Omega_22 = 0;
    for i = 1:N
        Omega_22 = Omega_22 + Q(i,i)*W1(i,i)*4*(rd_hat(i)+ d_ij(1,i))^2 ;
    end
...
%     r_hat = psi1(4:end);
%     rd_hat = psi1;
    W1 = compute_W1(u_hat, Tx, Rx, r_hat, AOA_theta, AOA_phi, sigma_r, sigma_AOA);
end
    % 第一阶段WLS估计
